...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self, images_dir, labels_dir, batch_size, image_width, image_height, use_augmentation):
        ...
        self.labels = self.__get_only_vertical(labels_dir, images_dir)
    ...
    @staticmethod
    def __get_only_vertical(labels_dir, images_dir):
        labels = []
        ...
        for i, f in enumerate(labels_temp):
            ...
            if np.random.randint(0, 21) == 20:
                ...
                while self.image_height >= rescaled_height or self.image_width >= rescaled_width:
                    ...
                    rescaled_width = scale * orig_width
                    rescaled_height = scale * orig_height
                image = cv2.resize(image, (rescaled_width, rescaled_height))
                ...
                bboxes = list(filter(lambda box: box[3] - box[1] > box[2] - box[0], bboxes))
                ...
                # Place image region centred on black background with aspect ratio 1:2
                # Pad top and bottom of cutout to fit aspect ratio
                y_margin = 0
                x_margin = 0
                if cutout.shape[1] * 2 > cutout.shape[0]:
                    y_margin = ((cutout.shape[1] * 2) - cutout.shape[0]) / 2
                elif cutout.shape[1] * 2 < cutout.shape[0]:
                    x_margin = (cutout.shape[0] - cutout.shape[1]) / 2
