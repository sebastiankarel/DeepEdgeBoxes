import keras
from DataGenBinaryClassificationVertical import DataGenerator
from DataGenBinaryClassificationHEDVertical import DataGenerator as DataGenHed
import os
import cv2
import numpy as np
...
class Classification:
    ...
    def __init__(self, image_width, image_height, class_weights, weight_file, use_hed, use_multichannel, use_rgb, hed=None):
        if image_height is not image_height * 2:
    ...
    def predict(self, image, threshold=0.2):
        ...
        scales = range(1, 8)
        x_overlap = 0.5
        y_overlap = x_overlap / 2.0  # Window aspect ration 1:2 -> move slower in y direction by factor 2
        for scale in scales:
            image_width = round(scale * window_width)
            image_height = round(scale * window_width)  # Image must remain square
            ...
            steps = int(((scale / x_overlap) - 1))
            for x in range(0, steps):
                x_offset = x * (window_width * x_overlap)
                for y in range(0, steps):
                    y_offset = y * (window_height * y_overlap)
                    xmin = int(x_offset)
